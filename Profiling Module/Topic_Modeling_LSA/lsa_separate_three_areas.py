...
files = ['ieee_dataset/machine_learning_dataset.csv', 'ieee_dataset/big_data_dataset.csv', 'ieee_dataset/computer_vision_dataset.csv']
concept_words = {}
count = 1
...
with open('data_lsa/test_user_final.csv', encoding="utf-8") as input_file:
    readCSV = csv.reader(input_file, delimiter=',')
    for row in readCSV:
        test.append(row[1])
...
for score in all_scores:
    if(count <= (results_len/3)*1):
        ml.append(score)
        count = count + 1
    elif(count <= (results_len/3)*2):
        bd.append(score)
        count = count + 1
    elif(count <= (results_len/3)*3):
        img.append(score)
        count = count + 1
    else:
...
for i in range(0, int(results_len/3)):
    # if(ml[i] == 0 and bd[i] == 0 and img[i] == 0):
    #     break

    if(ml[i] > bd[i] and ml[i] > img[i]):
        a.append(ml[i])

    elif(bd[i] > ml[i] and bd[i] > img[i]):
        b.append(bd[i])

    elif(img[i] > ml[i] and img[i] > bd[i]):
        c.append(img[i])
...
ml_score = (ml_len / int(results_len/3)) * 100
bd_score = (bd_len / int(results_len/3)) * 100
img_score = (img_len / int(results_len/3)) * 100

print(str(ml_score) + ', ' + str(bd_score) + ', ' + str(img_score))
